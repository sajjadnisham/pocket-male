...
| `city.html` | **Pocket Malé City** — real Malé in 3D, built from OpenStreetMap. Walk, run or fly; go inside 14 situations placed where they really are. |
| `city-data.json` | Malé's coastline, streets, buildings and situation locations, generated by `tools/build-city-data.js`. |
...
## Pocket Malé City (real Malé in 3D)

`city.html` builds Malé island at load time from `city-data.json`:

- **Streets, coastline, parks and 917 mapped buildings** come from OpenStreetMap.
  OSM maps only part of Malé's buildings, so the remaining city blocks are
  filled with generic buildings (the intro says so).
- **14 situations sit at real locations**: the Fish Market, the Local Market,
  IGMH, the Greater Malé Bus Terminal and a ferry jetty by name; the tea shop,
  restaurant, bank, pharmacy, barber, phone shop, corner shop and taxi office
  at the locations of real businesses of that kind, **deliberately unnamed** so
  scripted dialogue is never attributed to a real shop.
- Walk up to a glowing door and **go inside** — each situation has an interior
  with someone to talk to and things to tap.
- **Run**, **Fly** (with altitude control), a live **minimap**, a full map and a
  **Places** list for crossing the island. Guided mode tours all 14 in
  nearest-neighbour order.
- Buildings and roads are merged into a few meshes (≈92k triangles, ≈75 draw
  calls) and share a collision grid.

### About Street View here

Google Street View is shown **in Google's own embedded viewer** — "Street View
here" and "Live Street View" (which refreshes to the nearest photosphere when
you stop walking). The imagery is not downloaded, copied or turned into 3D:
Google's terms don't allow that, and Malé has no continuous Street View
coverage to move through in any case — only scattered user photospheres.
